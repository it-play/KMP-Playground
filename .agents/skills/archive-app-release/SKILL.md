...
- **tag** — no semantic versioning: a short lowercase slug naming the app (`meal`, `bodyfit`).
- **display name** — the app's name, used as the release title. May be Korean.

If either is missing, infer from the codebase and confirm with the user before tagging.
